...
public static class CSharpFormatter
{
    ...
    )
    {
        ...
        // Convert block-scoped namespaces to file-scoped if configured
        if (printerOptions.PreferFileScopedNamespace)
        {
            var rewritten = FileScopedNamespaceRewriter.Rewrite(rootNode);
            if (rewritten is CompilationUnitSyntax rewrittenRoot)
            {
                rootNode = rewrittenRoot;
                syntaxTree = rootNode.SyntaxTree;
            }
        }
